...
module.exports = function getCollectionFrom(db, storeName, limit, indexName, direction, boundType, filter) {
  ...
  return Q.Promise(function(resolve, reject) {
    ...
    transaction.onsuccess = function(e) {
      ...
      if (filter) {
        ...
        var check = Object.keys(filter).reduce(function(memo, key) {
          console.log(filter[key] === cursor.value[key]);
          return memo && filter[key] === cursor.value[key];
        }, true);

        if (check) results.push(cursor.value);
